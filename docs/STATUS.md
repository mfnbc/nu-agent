...
- **`engine.nu`** — `run contract prompt` dispatches by `action.verb`:
  - **Enrich** — single-shot JSON fill-in. Engine sends `[system, user]` to the LLM and returns the raw response string. No corpus retrieval, no tool loop. The contract's system prompt declares which fields to fill in; the user message IS the JSON record; the caller parses and validates the response. Designed for structured enrichment pipelines (e.g. adding gloss and cognate data to a Hebrew root registry) where the output must be machine-readable, not prose.
