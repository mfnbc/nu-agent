...
A **contract** narrows the aperture of a single nu-agent invocation along **two dimensions**:

- **Role** — who the LLM is. A Domain (chess, nutrition, ledger, software) and a Persona within it: Operator, Consultant, Developer, or a domain-specialised role like Nutritionist or Chess-Coach.
- **Action Scope** — what the LLM may do. An Action verb (Consult, Investigate, Enact), the Corpus it may read, the Tool-set it may call, the Output-shape it must produce, and the Execution-shape governing how its output is dispatched.

See [CONTRACTS.md](CONTRACTS.md) for the full breakdown.
